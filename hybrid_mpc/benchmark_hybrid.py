...
def compute_metrics(states, refs, times, solve_times, solve_ok):
...
def run_episode(traj_name, duration):
    ...
    metrics=compute_metrics(states,refs,times,solve_times,solve_ok)
...
if __name__=="__main__":
    ...
    for traj in TRAJS:
        ...
        print(f"  pos_rmse={m['pos_rmse']:.4f}m  "
              ...
              f"fail={m['fail_rate']*100:.1f}%")

    label = f"Hybrid MPC ({'NN+physics' if USE_NN else 'physics only'})"
    plot_all_trajectories(all_results,
                          controller_label=label,
                          save_prefix="benchmark_hybrid",
                          color="#1D9E75")